# Estimating Individual Tree Mortality in the Sierra Nevada Using Lidar and Hyperspectral Data
Supporting scripts and datasets for Hemming-Schroeder et al. manuscript 
...
* data/deliverables/point_data
    ...
    + trees_YYYY_ndvi.shp
        ...
        - poly_area is the crown area for the year given by YYYY in the filename
    ...
    + stovall_matches.csv
        - The first 24 columns are from Stovall et al. (2019) or artifacts from reading in the data (e.g. X.1). Our additions are appended at the end starting with d through contain.
        - d is the distance (meters) from their tree to our nearest tree in meters
        - treeID is the tree ID of our nearest tree
        - zmax2013 is the height of our nearest tree in 2013 (in meters)
        - label is our label for the tree (live=1, dead=0)
        - contain is whether or not our tree perimeter contains the coordinate of the Stovall tree location. The tree must be contained by our tree's crown perimeter for them to be a match. 
...
* data/deliverables/site_outlines
    + Shapefiles to describe the outlines of the Soaproot Saddle and Lower Teakettle lidar data intersection for the years 2013, 2017, 2018, 2019, and 2021
...
Scripts for figures and analysis discussed in the paper (still being updated)

## hpc/sierra directory

Files for this project created for use on a high performance computing cluster to create the datasets in data/deliverables (still being generalized)
