...
#home_dir0 <- home_dir
#lidar_dir <- "/Volumes/LaCie/NEON/lidar"
#task_idx <- c("8633", "8635") # TESTING ONLY

# HPC3
home_dir <- "/dfs4/jranders_lab/users/hemmingn/sierra/align_trees"
home_dir0 <- "/data/homezvol0/hemmingn/sierra/align_trees"
lidar_dir <- "/dfs4/jranders_lab/users/hemmingn/NEON/lidar"
...
idx_df <- read.csv(paste0(home_dir0, "/align_trees_indices.csv"))
...
for (i in 1:N){
  ...
  if (length(idx) > 0){
    ...
    if (!skip_to_next_1){
      ...
      str(las_temp)
      print("Mutate year")
      las_year <- c(2013, times = dim(las_temp)[1])
      las_temp <- cbind.data.frame(las_temp, las_year)
      str(las_temp)
      las_trees[[it]] <- dplyr::select(las_temp, X, Y, Z, treeID, las_year)
      it <- it + 1
    }
    ...
    if (!skip_to_next_2){
      ...
      str(las_temp)
      las_year <- c(2017, times	= dim(las_temp)[1])
      las_temp <- cbind.data.frame(las_temp, las_year)
      las_trees[[it]] <- dplyr::select(las_temp, X, Y, Z, treeID, las_year)
      it <- it + 1
    }
    ...
    if (!skip_to_next_3){
      ...
      str(las_temp)
      las_year <- c(2018, times	= dim(las_temp)[1])
      las_temp <- cbind.data.frame(las_temp, las_year)
      las_trees[[it]] <- dplyr::select(las_temp, X, Y, Z, treeID, las_year)
      it <- it + 1
    }
    ...
    if (!skip_to_next_4){
      ...
      str(las_temp)
      las_year <- c(2019, times = dim(las_temp)[1])
      las_temp <- cbind.data.frame(las_temp, las_year)
      las_trees[[it]] <- dplyr::select(las_temp, X, Y, Z, treeID, las_year)
      it <- it + 1
    }
    ...
    if (!skip_to_next_5){
      ...
      str(las_temp)
      las_year <- c(2021, times = dim(las_temp)[1])
      las_temp <- cbind.data.frame(las_temp, las_year)
      las_trees[[it]] <- dplyr::select(las_temp, X, Y, Z, treeID, las_year)
    }
...
if (flag != 0){
  ...
  if (!all(is.na(tree_objects$zmax2013))){
    ...
    if (length(fname_check) == 0){
      ...
      st_write(tree_objects_2013, fname, update = FALSE, delete_dsn = TRUE)
    }
  }
  ...
  if (!all(is.na(tree_objects$zmax2017))){
    ...
    if (length(fname_check) == 0){
      ...
      st_write(tree_objects_2017, fname, update = FALSE, delete_dsn = TRUE)
  ...
  if (!all(is.na(tree_objects$zmax2018))){ 
    ...
    if (length(fname_check) == 0){
      ...
      st_write(tree_objects_2018, fname, update = FALSE, delete_dsn = TRUE)
  ...
  if (!all(is.na(tree_objects$zmax2019))){
    ...
    if (length(fname_check) == 0){
      ...
      st_write(tree_objects_2019, fname, update = FALSE, delete_dsn = TRUE)
  ...
  if (!all(is.na(tree_objects$zmax2021))){
    ...
    if (length(fname_check) == 0){
      ...
      st_write(tree_objects_2021, fname, update = FALSE, delete_dsn = TRUE)
