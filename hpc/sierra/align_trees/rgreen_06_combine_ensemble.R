...
home_dir <- "/Volumes/LaCie/sierra/figures/data/ensemble"
home_dir <- "/dfs5/bio/hemmingn/sierra/align_trees"
setwd(home_dir)
...
fnames <- list.files(path = home_dir, pattern = glob2rx(paste0(veg_idx,"_optim_params_train_*.csv")), full.names = TRUE)
...
str(optim_params)
tail(optim_params)
write.csv(optim_params, paste0(veg_idx, "_optim_params_ensemble_train.csv"))

fnames2 <- list.files(path = home_dir, pattern = glob2rx(paste0(veg_idx, "_optim_params_valid_*.csv")), full.names = TRUE)
...
write.csv(optim_params2, paste0(veg_idx, "_optim_params_ensemble_valid.csv"))
...
p1
ggsave(paste0(veg_idx, "_accuracy_training_labels_v2.png"), dpi = 600)
...
p2
ggsave(paste0(veg_idx, "_accuracy_validation_labels_v2.png"), dpi = 600)
...
p1 <- ggplot(data = optim_params_groups) + 
  ...
  xlab("Number of labeled training data") + ylab("Training Set Accuracy") + ylim(c(0.70, 1.01)) 
p1
ggsave(paste0(veg_idx, "_accuracy_training_labels_v3.png"), dpi = 600)
...
p2 <- ggplot(data = optim_params_groups2) + 
  ...
  xlab("Number of labeled training data") + ylab("Validation Set Accuracy") + ylim(c(0.70, 1.01)) 
p2
ggsave(paste0(veg_idx, "_accuracy_validation_labels_v3.png"), dpi = 600)

plot_grid(p1, p2, labels = c('A', 'B'), label_size = 12)
ggsave(paste0(veg_idx, "_accuracy_panel_v3.png"), width = 6, height = 3, units = "in", dpi = 600)
...
ggsave(paste0(veg_idx, "_rdead_training_labels.png"), dpi = 600)
