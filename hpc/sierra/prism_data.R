...
# HPC3
out_dir <- "/dfs4/jranders_lab/users/hemmingn/sierra/prism"
data_dir <- "/dfs4/jranders_lab/users/hemmingn/PRISM"
landsat_dir <- "/dfs4/jranders_lab/users/hemmingn/Landsat"
home_dir <- "/data/homezvol0/hemmingn/sierra"
home_dir_2 <- "/dfs5/bio/hemmingn/sierra/align_trees"
setwd(home_dir)

# Set theme for images
theme_set(theme_bw(base_size = 12))

fnames <- list.files(data_dir, pattern = glob2rx(paste0("*", prism_var, "*.bil")), full.names = TRUE, recursive = TRUE)
...
lidar_bbox <- read_sf(paste0(home_dir_2, "/neon_las_extent.shp"))
